...
use anyhow::Result;
use fs_err as fs;
use leaf::acp::PermissionDecision;
    ...
    for &name in LeafMode::VARIANTS {
        let leaf_mode: LeafMode = name.parse().map_err(|_| {
            sacp::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse LeafMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = leaf_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
impl LeafAcpAgent {
    ...
    async fn create_agent_for_session(
        ...
        let mode = leaf_mode.unwrap_or(self.leaf_mode);
        ...
        let mut extensions = Config::new(&config_path, "leaf")
            .ok()
...
impl LeafAcpAgent {
    ...
    async fn on_prompt(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(leaf::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions.get_mut(&session_id).ok_or_else(|| {
                        sacp::Error::invalid_params()
    ...
    async fn on_set_mode(
        ...
        let mode = mode_id.parse::<LeafMode>().map_err(|_| {
            sacp::Error::invalid_params().data(format!("Invalid mode: {}", mode_id))
        })?;

        let agent = self.get_session_agent(session_id, None).await?;
        agent
            .update_leaf_mode(mode, session_id)
            .await
            .map_err(|e| {
...
mod tests {
    use super::*;
    use leaf::conversation::message::{ToolRequest, ToolResponse};
...
```",
    ...
    impl leaf::providers::base::Provider for MockModelProvider {
        ...
        async fn stream(
            &self,
            _model_config: &leaf::model::ModelConfig,
            _session_id: &str,
            _system: &str,
            _messages: &[leaf::conversation::message::Message],
            _tools: &[rmcp::model::Tool],
        ) -> Result<leaf::providers::base::MessageStream, ProviderError> {
            unimplemented!()
        }

        fn get_model_config(&self) -> leaf::model::ModelConfig {
            leaf::model::ModelConfig::new_or_fail("unused")
        }
