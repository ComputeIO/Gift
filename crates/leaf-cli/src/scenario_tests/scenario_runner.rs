...
use anyhow::Result;
use leaf::agents::{Agent, AgentConfig, LeafPlatform};
use leaf::config::permission::PermissionManager;
use leaf::config::LeafMode;
use leaf::model::ModelConfig;
use leaf::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("LEAF_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    leaf::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn leaf::providers::base::Provider>,
            &session.id,
        )
        .await?;
