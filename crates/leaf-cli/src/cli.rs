...
use leaf::posthog::get_telemetry_choice;
use leaf::recipe::Recipe;
use leaf_mcp::mcp_server_runner::{serve, McpCommand};
use leaf_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
#[derive(Parser)]
#[command(name = "leaf", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[arg(
        short = 'v',
        long,
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), leaf::config::DEFAULT_EXTENSION_TIMEOUT);
...
enum RecipeCommand {
    ...
    /// Open a recipe in Leaf Desktop
    #[command(about = "Open a recipe in Leaf Desktop")]
    Open {
...
enum Command {
    ...
    Completion {
        ...
        #[arg(long, default_value = "leaf", help = "Provide a custom binary name")]
        bin_name: String,
    },
...
async fn handle_interactive_session(
    ...
    tracing::info!(
        monotonic_counter.leaf.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.leaf.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.leaf.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.leaf.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    leaf::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.leaf.recipe_runs = 1,
...
pub async fn run_cli(cli: Cli) -> anyhow::Result<()> {
    ...
    tracing::info!(
        monotonic_counter.leaf.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::ValidateExtensions { file }) => {
            use leaf::agents::validate_extensions::validate_bundled_extensions;
