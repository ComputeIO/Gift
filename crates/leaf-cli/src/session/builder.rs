...
use console::style;
use leaf::agents::{Agent, Container, ExtensionError};
use leaf::config::resolve_extensions_for_new_session;
use leaf::config::{get_all_extensions, Config, ExtensionConfig, LeafMode};
use leaf::providers::create;
use leaf::recipe::Recipe;
use leaf::session::session_manager::SessionType;
use leaf::session::EnabledExtensionsState;
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn leaf::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
...
async fn load_extensions(
    ...
    provider_for_debug: Arc<dyn leaf::providers::base::Provider>,
    interactive: bool,
    session_id: &str,
) -> Arc<Agent> {
...
struct ResolvedProviderConfig {
    ...
    model_name: String,
    model_config: leaf::model::ModelConfig,
}
...
fn resolve_provider_and_model(
    ...
    saved_model_config: Option<leaf::model::ModelConfig>,
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &leaf::session::session_manager::SessionManager,
    leaf_mode: LeafMode,
) -> String {
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    leaf::posthog::set_session_context("cli", session_config.resume);
    ...
    let session_id =
        resolve_session_id(&session_config, &session_manager, agent.config.leaf_mode).await;
    ...
    agent
        .update_leaf_mode(agent.config.leaf_mode, &session_id)
        .await
        .unwrap_or_else(|e| {
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: leaf::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
