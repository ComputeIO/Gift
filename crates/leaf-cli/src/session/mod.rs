...
use leaf::conversation::Conversation;
use std::io::Write;
use std::str::FromStr;
...
use console::Color;
use leaf::agents::AgentEvent;
use leaf::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use leaf::permission::permission_confirmation::PrincipalType;
use leaf::permission::Permission;
use leaf::permission::PermissionConfirmation;
...
use leaf::agents::{Agent, SessionConfig, COMPACT_TRIGGERS};
use leaf::config::extensions::name_to_key;
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: leaf::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(leaf::config::DEFAULT_EXTENSION_TIMEOUT),
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            leaf::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    fn handle_leaf_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match LeafMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    LeafMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
    ...
    pub async fn get_session(&self) -> Result<leaf::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("LEAF_CLI_SHOW_COST")
            .unwrap_or(false);
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.leaf.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.leaf.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<leaf::providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                leaf::providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("LEAF_PLANNER_PROVIDER") {
        provider
    } else {
    ...
    let model = if let Ok(model) = config.get_param::<String>("LEAF_PLANNER_MODEL") {
        model
    } else {
...
mod tests {
    use super::*;
    use leaf::agents::extension::Envs;
    use leaf::config::ExtensionConfig;
