...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("leaf");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run leaf. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("leaf Project Manager");
    ...
    match choice {
        ...
        "fresh" => {
            ...
            // Build the command to run leaf with a fresh session
            let mut command = std::process::Command::new("leaf");
            command.arg("session");
