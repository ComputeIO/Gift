...
use leaf::agents::extension_manager::get_parameter_names;
use leaf::agents::Agent;
use leaf::agents::{extension::Envs, ExtensionConfig};
use leaf::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use leaf::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use leaf::config::paths::Paths;
use leaf::config::permission::PermissionLevel;
use leaf::config::signup_tetrate::TetrateAuth;
use leaf::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, LeafMode,
    PermissionManager,
};
use leaf::model::ModelConfig;
use leaf::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use leaf::providers::base::ConfigKey;
use leaf::providers::chatgpt_codex::reasoning_levels_for_model;
use leaf::providers::formats::anthropic::supports_adaptive_thinking;
use leaf::providers::provider_test::test_provider_configuration;
use leaf::providers::{create, providers, retry_operation, RetryConfig};
use leaf::session::SessionType;
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve leaf").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve leaf by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how leaf is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • leaf version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'leaf configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve leaf?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve leaf!");
    } else {
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &leaf::providers::base::ProviderMetadata,
    ...
    if sorted_models.len() > MAX_MODELS {
        ...
        // This ensures all models from the API are shown, not just those
        // that match the provider's npm group in the catalog
        let mut recommended_models: Vec<String> = provider_meta
            .known_models
            .iter()
            .map(|m| m.name.clone())
            .filter(|name| sorted_models.contains(name))
            .collect();

        // Add any models from API that aren't in known_models
        // (e.g., models from other npm groups that are also available)
        for model in &sorted_models {
            if !recommended_models.contains(model) {
                recommended_models.push(model.clone());
            }
        }
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&leaf::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_leaf_mode_dialog() -> anyhow::Result<()> {
    ...
    let msg = match mode {
        ...
        LeafMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        LeafMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve leaf!")?;
    } else {
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use leaf::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(leaf::config::DEFAULT_DISPLAY_NAME.to_string()),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                        });
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = leaf::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        leaf::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = leaf::providers::cleanup_provider(selected_id).await {
