...
use leaf::config::declarative_providers::LoadedProvider;
use leaf::config::paths::Paths;
use leaf::config::ExtensionEntry;
use leaf::config::{Config, ConfigError};
use leaf::model::ModelConfig;
use leaf::providers::auto_detect::detect_provider_from_api_key;
use leaf::providers::base::{ProviderMetadata, ProviderType};
use leaf::providers::canonical::maybe_get_canonical_model;
use leaf::providers::catalog::{
    get_provider_template, get_providers_by_format, ProviderCatalogEntry, ProviderFormat,
    ProviderTemplate,
};
use leaf::providers::create_with_default_model;
use leaf::providers::providers as get_providers;
use leaf::{
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, ErrorResponse> {
    let extensions = leaf::config::get_all_extensions();
    let warnings = leaf::config::get_warnings();
...
pub async fn add_extension(
    ...
    let extensions = leaf::config::get_all_extensions();
    let key = leaf::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    leaf::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, ErrorResponse> {
    let key = leaf::config::extensions::name_to_key(&name);
    leaf::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let provider = leaf::providers::create(&name, model_config, Vec::new()).await?;
...
pub async fn init_config() -> Result<Json<String>, ErrorResponse> {
    ...
    match leaf::config::base::load_init_config_from_workspace() {
        Ok(init_values) => {
...
pub async fn upsert_permissions(
    ...
    let permission_manager = leaf::config::PermissionManager::instance();
...
pub async fn create_custom_provider(
    ...
    let config = leaf::config::declarative_providers::create_custom_provider(
        leaf::config::declarative_providers::CreateCustomProviderParams {
    ...
    leaf::providers::refresh_custom_providers().await?;
...
pub async fn get_custom_provider(
    ...
    let loaded_provider =
        leaf::config::declarative_providers::load_provider(id.as_str()).map_err(|e| {
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    leaf::config::declarative_providers::remove_custom_provider(&id)?;
...
pub async fn cleanup_provider_cache(
    ...
    leaf::providers::cleanup_provider(&name).await?;
...
pub async fn update_custom_provider(
    ...
    leaf::config::declarative_providers::update_custom_provider(
        leaf::config::declarative_providers::UpdateCustomProviderParams {
            id: id.clone(),
            engine: request.engine,
            display_name: request.display_name,
            api_url: request.api_url,
