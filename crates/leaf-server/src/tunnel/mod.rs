...
use leaf::config::{paths::Paths, Config};
...
fn try_acquire_tunnel_lock() -> anyhow::Result<File> {
    ...
    file.try_lock_exclusive()
        .map_err(|_| anyhow::anyhow!("Another leaf instance is already running the tunnel"))?;
    ...
    pub async fn check_auto_start(&self) {
        ...
        if auto_start && state == TunnelState::Idle {
            if is_tunnel_locked_by_another() {
                tracing::info!(
                    "Tunnel already running on another leaf instance, skipping auto-start"
                );
                return;
            }
