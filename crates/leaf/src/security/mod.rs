...
impl SecurityManager {
    ...
    pub async fn analyze_tool_requests(
        ...
        if !self.is_prompt_injection_detection_enabled() {
            tracing::debug!(
                monotonic_counter.leaf.prompt_injection_scanner_disabled = 1,
        ...
        let scanner = self.scanner.get_or_init(|| {
            ...
            tracing::info!(
                monotonic_counter.leaf.security_command_classifier_enabled = if command_classifier_enabled { 1 } else { 0 },
                monotonic_counter.leaf.security_prompt_classifier_enabled = if prompt_classifier_enabled { 1 } else { 0 },
            ...
            let scanner = if ml_enabled {
                match PromptInjectionScanner::with_ml_detection() {
                    Ok(s) => {
                        tracing::info!(
                            monotonic_counter.leaf.prompt_injection_scanner_enabled = 1,
        ...
        for tool_request in tool_requests.iter() {
            if let Ok(tool_call) = &tool_request.tool_call {
                ...
                if analysis_result.is_malicious {
                    ...
                    tracing::warn!(
                        monotonic_counter.leaf.prompt_injection_finding = 1,
                    ...
                    tracing::info!(
                        monotonic_counter.leaf.prompt_injection_tool_call_passed = 1,
                        tool_name = %tool_call.name,
                        tool_request_id = %tool_request.id,
                        tool_call_json = %tool_call_json,
                        confidence = analysis_result.confidence,
        ...
        tracing::info!(
            monotonic_counter.leaf.prompt_injection_analysis_performed = 1,
