...
use crate::config::{get_enabled_extensions, Config, LeafMode};
...
use rmcp::model::{
    ...
    JsonObject, Prompt, ServerNotification, Tool,
};
...
pub struct Agent {
    ...
    pub(super) current_leaf_mode: Mutex<LeafMode>,
...
impl Agent {
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        Self {
            ...
            config,
            current_leaf_mode: Mutex::new(initial_mode),
    ...
    async fn prepare_reply_context(
        ...
        let leaf_mode = *self.current_leaf_mode.lock().await;
        ...
        for request in &permission_check_result.denied {
            if let Some(response) = request_to_response_map.get_mut(&request.id) {
                response.add_tool_response_with_metadata(
                    ...
                    match request.tool_call {
                        Ok(ref call) => call.arguments.clone(),
                        Err(_) => None,
                    },
                );
            }
    ...
    async fn reply_internal(
        ...
        let reply_stream_span = tracing::info_span!(target: "leaf::agents::agent", "reply_stream", session.id = %session_config.id);
        let inner = Box::pin(async_stream::try_stream! {
                    ...
                    match next {
                        Ok((response, usage)) => {
                            ...
                            if let Some(response) = response {
                                ...
                                let mut request_tool_args: HashMap<String, JsonObject> = HashMap::new();
                                for request in frontend_requests.iter().chain(remaining_requests.iter()) {
                                    ...
                                    if let Ok(ref call) = request.tool_call {
                                        ...
                                        if let Some(ref args) = call.arguments {
                                            request_tool_args.insert(request.id.clone(), args.clone());
                                        };
                                    }
                                }
                                                        ...
                                                        match item {
                                                            ToolStreamItem::Result(output) => {
                                                                ...
                                                                if let Some(response) = request_to_response_map.get_mut(&request_id) {
                                                                    ...
                                                                    let tool_args = request_tool_args.get(&request_id).cloned();
                                                                    response.add_tool_response_with_metadata(request_id, output, metadata, tool_name, tool_args);
                                                                }
                                                            }
            ...
            if !last_assistant_text.is_empty() {
                tracing::info!(target: "leaf::agents::agent", trace_output = last_assistant_text.as_str());
            }
    ...
    pub async fn update_leaf_mode(&self, mode: LeafMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_leaf_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist leaf_mode to session")
    }

    pub async fn leaf_mode(&self) -> LeafMode {
    ...
    pub async fn restore_provider_from_session(&self, session: &Session) -> Result<bool> {
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.leaf_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_leaf_mode.lock().await = session.leaf_mode;
        Ok(provider_changed)
    }
...
mod tests {
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_leaf_mode(LeafMode::default())
            .build();
