...
pub const CHAT_MODE_TOOL_SKIPPED_RESPONSE: &str = "Let the user know the tool call was skipped in leaf chat mode. \
        ...
        for request in tool_requests.iter() {
            if let Ok(tool_call) = request.tool_call.clone() {
                ...
                if let Some(finding_id) = get_security_finding_id_from_results(&request.id, inspection_results) {
                    tracing::info!(
                        monotonic_counter.leaf.prompt_injection_user_decisions = 1,
