...
pub fn manage_schedule_tool() -> Tool {
    Tool::new(
        ...
        indoc! {r#"
            Manage leaf's internal scheduled recipe execution.

            Actions:
            - "list": List all leaf scheduled jobs
            - "create": Create a new leaf scheduled job from a recipe file
            - "run_now": Execute a leaf scheduled job immediately
            - "pause": Pause a leaf scheduled job
            - "unpause": Resume a paused leaf scheduled job
            - "delete": Remove a leaf scheduled job
            - "kill": Terminate a currently running leaf scheduled job
            - "inspect": Get details about a running leaf scheduled job
            - "sessions": List execution history for a leaf scheduled job
