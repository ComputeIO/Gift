...
async fn init_registry() -> RwLock<ProviderRegistry> {
    ...
    // Register OpenCode providers from models.dev catalog
    if let Err(e) = opencode::register_opencode_providers(&mut registry).await {
        tracing::warn!("Failed to register OpenCode providers: {}", e);
    }
...
pub fn get_registry_optional() -> Option<&'static RwLock<ProviderRegistry>> {
    REGISTRY.get()
}
