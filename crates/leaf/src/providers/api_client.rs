...
impl TlsConfig {
    ...
    pub fn from_config() -> Result<Option<Self>> {
        ...
        let client_cert_path = config.get_param::<String>("LEAF_CLIENT_CERT_PATH").ok();
        let client_key_path = config.get_param::<String>("LEAF_CLIENT_KEY_PATH").ok();
        ...
        match (client_cert_path, client_key_path) {
            ...
            (Some(_), None) => {
                return Err(anyhow::anyhow!(
                    "Client certificate provided (LEAF_CLIENT_CERT_PATH) but no private key (LEAF_CLIENT_KEY_PATH)"
                ));
            }
            (None, Some(_)) => {
                return Err(anyhow::anyhow!(
                    "Client private key provided (LEAF_CLIENT_KEY_PATH) but no certificate (LEAF_CLIENT_CERT_PATH)"
                ));
            }
            (None, None) => {}
        }

        if let Ok(ca_cert_path) = config.get_param::<String>("LEAF_CA_CERT_PATH") {
...
impl<'a> ApiRequestBuilder<'a> {
        ...
        F: FnOnce(url::Url, &Client) -> reqwest::RequestBuilder,
    {
        ...
        tracing::debug!(
            "API request: url={}, path={}, auth={:?}",
            url,
            self.path,
            match &self.client.auth {
                AuthMethod::NoAuth => "None",
                AuthMethod::BearerToken(_) => "BearerToken",
