...
impl OpenAiProvider {
    ...
    pub fn from_custom_config(
        ...
        // Free models use "public" as API key
        let is_free_model = model.model_name.to_lowercase().ends_with("-free");

        let api_key: Option<String> = if is_free_model {
            Some("public".to_string())
        } else if config.requires_auth && !config.api_key_env.is_empty() {
        ...
        let full_url = if base_path.starts_with('/') {
            format!("{}{}", host, base_path)
        } else {
            format!("{}/{}", host, base_path)
        };
        tracing::debug!(
            "Creating provider: name={}, host={}, base_path={}, model={}, is_free={}, has_auth={}, full_url={}",
            config.name,
            host,
            base_path,
            model.model_name,
            is_free_model,
            matches!(auth, AuthMethod::BearerToken(_)),
            full_url
        );
        ...
        Ok(Self {
            ...
            preloaded_models: if config.models.is_empty() {
                None
            } else {
...
impl Provider for OpenAiProvider {
    ...
    async fn fetch_supported_models(&self) -> Result<Vec<String>, ProviderError> {
        tracing::debug!(
            "fetch_supported_models: provider={}, preloaded_models={:?}",
            self.name,
        ...
        match response {
            Ok(resp) => {
                let json = handle_response_openai_compat(resp).await?;
            ...
            Err(e) => {
                tracing::debug!(
                    "API call failed for {}: {}, falling back to preloaded models",
                    self.name,
                    e
                );
            }
        ...
        // Fallback to preloaded models if API call fails
        if let Some(ref models) = self.preloaded_models {
            if !models.is_empty() {
                tracing::debug!(
                    "Returning {} preloaded models for {} (API unavailable)",
                    models.len(),
                    self.name
                );
                let sample: Vec<_> = models.iter().take(5).cloned().collect();
                tracing::debug!("Sample models: {:?}", sample);
                return Ok(models.clone());
            }
        }

        Err(ProviderError::RequestFailed(
            "No models available (API failed and no preloaded models)".to_string(),
        ))
    }
