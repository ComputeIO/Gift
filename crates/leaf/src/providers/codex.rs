...
use crate::config::{Config, ExtensionConfig, LeafMode};
...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, LeafMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on LeafMode
    fn apply_permission_flags(cmd: &mut Command, leaf_mode: LeafMode) -> Result<(), ProviderError> {
        match leaf_mode {
            LeafMode::Auto => {
    ...
    async fn execute_command(
        ...
        Self::apply_permission_flags(&mut cmd, leaf_mode)?;
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(system, messages, tools, leaf_mode)
            .await?;
...
mod tests {
    ...
    use leaf_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    ...
    #[test_case(LeafMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(LeafMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(LeafMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(LeafMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: LeafMode, expected: &[&str]) {
