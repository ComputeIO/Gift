...
mod tests {
    ...
    async fn test_zai_provider() {
        ...
        assert!(
            zai.is_some(),
            "z.ai should be registered as a native provider"
        );

        let (meta, provider_type) = zai.unwrap();
        println!("Z.AI provider type: {:?}", provider_type);
        assert!(
            !meta.default_model.is_empty(),
            "z.ai should have a default model"
        );
    }
}
