...
impl AnthropicProvider {
    ...
    pub fn from_custom_config(
        ...
        let (host, base_path) = if let Some(ref explicit_path) = config.base_path {
            ...
            (host, explicit_path.trim_start_matches('/').to_string())
        } else {
            let url = url::Url::parse(&config.base_url)
                .map_err(|e| anyhow::anyhow!("Invalid base URL '{}': {}", config.base_url, e))?;
...
impl Provider for AnthropicProvider {
    ...
    async fn stream(
        ...
        let response = self
            .with_retry(|| async {
                let path = format!("{}/messages", self.base_path);
                let mut request = self.api_client.request(Some(session_id), &path);
