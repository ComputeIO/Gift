...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<LeafMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow leaf to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match leaf_mode {
            ...
            LeafMode::SmartApprove | LeafMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            LeafMode::Chat => Ok(false),
        }
    }
...
impl Provider for ClaudeCodeProvider {
    ...
    async fn update_mode(&self, _session_id: &str, mode: LeafMode) -> Result<(), ProviderError> {
...
mod tests {
    ...
    use chrono::Utc;
    use leaf_test_support::session::TEST_SESSION_ID;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;
