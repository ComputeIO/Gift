...
use crate::config::{ExtensionConfig, LeafMode};
...
impl Provider for AcpProvider {
    ...
    async fn stream(
        ...
        let leaf_mode = *self
            .leaf_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("leaf_mode lock poisoned".into()))?;
            ...
            while let Some(update) = rx.recv().await {
                match update {
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        if let Some(decision) = permission_decision_from_mode(leaf_mode) {
...
impl AcpClientLoop {
    fn new(config: AcpProviderConfig, leaf_mode: Arc<Mutex<LeafMode>>) -> Self {
        Self {
            config,
            leaf_mode,
            ...
            .on_receive_notification(
                {
                    ...
                    async move |notification: SessionNotification, _cx| {
                        ...
                        // stream() reads leaf_mode at call time, so it must
                        // reflect any prior set_mode before the next prompt.
                        if let SessionUpdate::CurrentModeUpdate(update) = &notification.update {
                            if let Ok(mode) = LeafMode::from_str(&update.current_mode_id.0) {
                                if let Ok(mut guard) = leaf_mode.lock() {
                                    *guard = mode;
                                }
                            }
                        }
...
fn permission_decision_from_mode(leaf_mode: LeafMode) -> Option<PermissionDecision> {
    match leaf_mode {
        LeafMode::Auto => Some(PermissionDecision::AllowOnce),
        LeafMode::Chat => Some(PermissionDecision::RejectOnce),
        LeafMode::Approve | LeafMode::SmartApprove => None,
    }
}
