...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn leaf_mode(mut self, mode: LeafMode) -> Self {
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, leaf_mode)
            .await
    }
...
impl SessionStorage {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE sessions (
                ...
                leaf_mode TEXT NOT NULL DEFAULT 'auto'
            )
        "#,
        )
        .execute(pool)
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, leaf_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
        ...
        .bind(session.leaf_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn apply_migration(tx: &mut sqlx::Transaction<'_, Sqlite>, version: i32) -> Result<()> {
        match version {
            ...
            8 => {
                sqlx::query(
                    r#"
                    ALTER TABLE sessions ADD COLUMN leaf_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                )
                .execute(&mut **tx)
                .await?;
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.leaf_mode, "leaf_mode");
        ...
        if let Some(leaf_mode) = builder.leaf_mode {
            q = q.bind(leaf_mode.to_string());
        }
    ...
    async fn list_sessions_by_types(&self, types: Option<&[SessionType]>) -> Result<Vec<Session>> {
        ...
        let query = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.leaf_mode,
    ...
    async fn copy_session(
        ...
        // Preserve provider, model config, and leaf_mode from original session
        ...
        builder = builder.leaf_mode(original_session.leaf_mode);
...
mod tests {
    ...
    #[test_case(LeafMode::Approve)]
    #[test_case(LeafMode::SmartApprove)]
    #[test_case(LeafMode::Chat)]
    #[tokio::test]
    async fn test_leaf_mode_persists(mode: LeafMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        assert_eq!(reloaded.leaf_mode, mode);
    }

    #[tokio::test]
    async fn test_leaf_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .leaf_mode(LeafMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.leaf_mode, LeafMode::Approve);
    }

    #[tokio::test]
    async fn test_leaf_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sqlx::query("UPDATE sessions SET leaf_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
