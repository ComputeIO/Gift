use crate::agents::{Agent, AgentConfig, LeafPlatform};
...
impl AgentManager {
    ...
    pub async fn get_or_create_agent(&self, session_id: String) -> Result<Arc<Agent>> {
        ...
        if let Ok(session) = self.session_manager.get_session(&session_id, false).await {
            mode = session.leaf_mode;
            info!(leaf_mode = %mode, session_id = %session_id, "Session loaded");
        }
...
mod tests {
    ...
    #[test_case(LeafMode::Approve ; "approve")]
    #[test_case(LeafMode::Chat ; "chat")]
    #[test_case(LeafMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: LeafMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.leaf_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.leaf_mode().await, LeafMode::Approve);
        assert_eq!(a2.leaf_mode().await, LeafMode::Auto);
    }
}
