...
use crate::config::{LeafMode, PermissionManager};
    ...
    async fn inspect(
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match leaf_mode {
                    LeafMode::Chat => continue,
                    LeafMode::Auto => InspectionAction::Allow,
                    LeafMode::Approve | LeafMode::SmartApprove => {
                        ...
                        // 2. Check if it's a smart-approved tool (annotation or cached LLM decision)
                            ...
                            || (leaf_mode == LeafMode::SmartApprove
                        ...
                        } else if leaf_mode == LeafMode::SmartApprove
                            && permission_manager
...
mod tests {
    ...
    #[test_case(LeafMode::Auto, false, None, InspectionAction::Allow; "auto_allows")]
    #[test_case(LeafMode::SmartApprove, true, None, InspectionAction::Allow; "smart_approve_annotation_allows")]
    #[test_case(LeafMode::SmartApprove, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::Allow; "smart_approve_cached_allow")]
    #[test_case(LeafMode::SmartApprove, false, Some(PermissionLevel::AskBefore), InspectionAction::RequireApproval(None); "smart_approve_cached_ask")]
    #[test_case(LeafMode::SmartApprove, false, None, InspectionAction::RequireApproval(None); "smart_approve_unknown_defers")]
    #[test_case(LeafMode::Approve, false, None, InspectionAction::RequireApproval(None); "approve_requires_approval")]
    #[test_case(LeafMode::Approve, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "approve_ignores_cache")]
    #[tokio::test]
    async fn test_inspect_action(
        ...
        let results = inspector
            .inspect(leaf_test_support::TEST_SESSION_ID, &[req], &[], mode)
            .await
            .unwrap();
