...
use leaf::conversation::message::{Message, MessageContent, ToolRequest};
use leaf::security::adversary_inspector::AdversaryInspector;
use leaf::tool_inspection::ToolInspector;
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        leaf::tool_inspection::InspectionAction::Allow
    ));
