...
use leaf::providers::base::Provider as LeafProvider;

static TEST_PROVIDER: Lazy<Mutex<Option<Arc<dyn LeafProvider>>>> = Lazy::new(|| Mutex::new(None));
...
pub async fn set_test_provider(p: Arc<dyn LeafProvider>) {
    let mut guard = TEST_PROVIDER.lock().await;
    *guard = Some(p);
}

pub async fn get_test_provider() -> Option<Arc<dyn LeafProvider>> {
