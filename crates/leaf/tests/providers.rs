...
use leaf::providers::gemini_acp::GEMINI_ACP_DEFAULT_MODEL;
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: LeafMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("LEAF_MODE", Some(<&str>::from(mode)))];
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<leaf::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = match &self.image_model {
            Some(model) => {
                Some(leaf::model::ModelConfig::new(model)?.with_canonical_limits(&self.name))
            }
            None => None,
        };
        let response = self
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config = leaf::model::ModelConfig::new(alt)?.with_canonical_limits(&self.name);
    ...
    async fn test_mode_update(&self) -> Result<()> {
        ...
        self.agent
            .update_leaf_mode(LeafMode::Approve, &self.session_id)
            .await?;
...
async fn test_provider(config: ProviderTestConfig) -> Result<()> {
    ...
    let run_test = |mode: LeafMode| ProviderFixture::setup(&config, mode);

    if run_test(LeafMode::Auto).await.is_err() {
    ...
    let result: Result<()> = async {
        run_test(LeafMode::Auto).await?.test_model_listing().await?;
        run_test(LeafMode::Auto)
            .await?
            .test_basic_response()
            .await?;
        run_test(LeafMode::Auto).await?.test_tool_usage().await?;
        run_test(LeafMode::Auto)
            .await?
            .test_image_content_support()
            .await?;
        if config.model_switch_name.is_some() {
            run_test(LeafMode::Auto).await?.test_model_switch().await?;
        }
        ...
        if config.test_permissions {
            ...
            if config.test_smart_approve {
                run_test(LeafMode::SmartApprove)
                    .await?
        ...
        if config.test_mode_update {
            run_test(LeafMode::Auto).await?.test_mode_update().await?;
        }
        Ok(())
    }
    .await;
