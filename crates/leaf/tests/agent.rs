...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use leaf::agents::platform_tools::PLATFORM_MANAGE_SCHEDULE_TOOL_NAME;
        use leaf::agents::AgentConfig;
        use leaf::config::permission::PermissionManager;
        use leaf::config::LeafMode;
        use leaf::scheduler::{ScheduledJob, SchedulerError};
        use leaf::scheduler_trait::SchedulerTrait;
        use leaf::session::{Session, SessionManager};
    ...
    mod retry_tests {
        use super::*;
        use leaf::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use leaf::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use leaf::agents::SessionConfig;
        use leaf::config::LeafMode;
        use leaf::conversation::message::{Message, MessageContent};
        use leaf::model::ModelConfig;
        use leaf::providers::base::{
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _model: ModelConfig,
                _extensions: Vec<leaf::config::ExtensionConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let leaf::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    leaf::permission::PermissionConfirmation {
                                        principal_type: leaf::permission::permission_confirmation::PrincipalType::Tool,
                                        permission: leaf::permission::Permission::AllowOnce,
                                    }
                                ).await;
                            }
                        }
    ...
    mod extension_manager_tests {
        use super::*;
        use leaf::agents::extension::ExtensionConfig;
        use leaf::agents::platform_extensions::{
            MANAGE_EXTENSIONS_TOOL_NAME, SEARCH_AVAILABLE_EXTENSIONS_TOOL_NAME,
        };
        use leaf::agents::AgentConfig;
        use leaf::config::permission::PermissionManager;
        use leaf::config::LeafMode;
        use leaf::session::SessionManager;
    ...
    mod streaming_persistence_tests {
        use super::*;
        use async_trait::async_trait;
        use leaf::agents::{AgentConfig, SessionConfig};
        use leaf::config::permission::PermissionManager;
        use leaf::config::LeafMode;
        use leaf::conversation::message::Message;
        use leaf::model::ModelConfig;
