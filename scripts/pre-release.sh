...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/leaf.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/leaf.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/leaf.zip" -d "$TMPDIR/extracted"
