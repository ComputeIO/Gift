...
A network-level HTTP proxy for simulating provider errors when testing Leaf's error handling and retry logic.

## Features

- **Interactive error injection**: Manually trigger different error types via stdin commands
- **Network-level interception**: No changes to Leaf's Rust code required
...
# 2. In another terminal, configure Leaf to use the proxy
...
# 3. Run Leaf normally
leaf session start "tell me a joke"
...
Then run Leaf normally. The proxy will intercept API requests and you can manually trigger errors as needed, while authentication and metadata requests are forwarded to the real provider.
...
To use with Leaf, set these environment variables:
  export OPENAI_HOST=http://localhost:8888
  export ANTHROPIC_HOST=http://localhost:8888
  ...
============================================================
...
2. Configure Leaf to use the proxy (set environment variables)
3. Run Leaf in another terminal
4. Use interactive commands to trigger different error types
5. Observe how Leaf handles each error type
