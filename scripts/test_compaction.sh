...
validate_compaction() {
  ...
  local session_json=$($LEAF_BIN session export --format json --session-id "$session_id" 2>&1)
...
(cd "$TESTDIR" && "$LEAF_BIN" run --with-builtin developer --text "list files and read hello.txt" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  SESSION_ID=$("$LEAF_BIN" session list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$LEAF_BIN" run --resume --session-id "$SESSION_ID" --text "Please compact this conversation" 2>&1) | tee -a "$OUTPUT"
...
export LEAF_AUTO_COMPACT_THRESHOLD=0.005
...
(cd "$TESTDIR" && "$LEAF_BIN" run --text "$LONG_RESPONSE_PROMPT" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$LEAF_BIN" run --resume --session-id "$SESSION_ID" --text "hi again" 2>&1) | tee -a "$OUTPUT"
...
unset LEAF_AUTO_COMPACT_THRESHOLD
...
if ! (cd "$PROXY_DIR" && uv sync 2>&1 | tee "$PROXY_SETUP_LOG"); then
  ...
  if [ "$PROXY_READY" != "true" ]; then
    ...
    export LEAF_PROVIDER_SKIP_BACKOFF=true
    export LEAF_PROVIDER=anthropic
    export LEAF_MODEL=claude-haiku-4-5

    echo "Step 1: Creating session (should trigger context-length error and compaction)..."
    (cd "$TESTDIR" && "$LEAF_BIN" run --text "hello world" 2>&1) | tee "$OUTPUT"
    ...
    unset LEAF_PROVIDER_SKIP_BACKOFF
    unset LEAF_PROVIDER
    unset LEAF_MODEL
    unset UV_INDEX_URL
  fi
