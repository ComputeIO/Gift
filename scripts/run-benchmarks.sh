...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting LEAF_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets LEAF_TOOLSHIM_MODEL)"
...
for ((i=0; i<$COUNT; i++)); do
  ...
  if [ "$TOOLSHIM" = true ]; then
    export LEAF_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export LEAF_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $LEAF_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
