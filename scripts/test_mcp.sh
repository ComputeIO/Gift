...
TEST_PROVIDER=${LEAF_PROVIDER:-anthropic}
...
TMPFILE=$(mktemp)
(cd "$TESTDIR" && LEAF_PROVIDER="$TEST_PROVIDER" LEAF_MODEL="$TEST_MODEL" \
    "$LEAF_BIN" run --recipe recipe.yaml 2>&1) | tee "$TMPFILE"
...
(cd "$TESTDIR" && LEAF_PROVIDER="$TEST_PROVIDER" LEAF_MODEL="$TEST_MODEL" \
    "$LEAF_BIN" run --text "Use the sampleLLM tool to ask for an original short poem about the ocean" \
...
EOF
    ...
    JUDGE_OUT=$(LEAF_PROVIDER="$TEST_PROVIDER" LEAF_MODEL="$TEST_MODEL" \
        "$LEAF_BIN" run --text "$JUDGE_PROMPT" 2>&1)
